...
**Status:** pre-draft / not yet posted. Section 1 (the mechanism) and §2's core compounding-drift result are settled. §2.1 is a follow-up sweep, not yet fully rigorous (see its own caveats), that surfaces a separate magnitude-driven error term the original §2 test didn't exercise — treat §2 as settled only for the specific claim it measured (compounding, fixed P), not as a complete precision picture. **Section 5's 7 original tensor-level questions are all answered (§5.1–§5.7), pending only end-to-end (not isolated) latency measurement and CUDA graph capture as noted throughout. Real-model recall accuracy (§5.new) is a separate, single-run result — not yet replicated, not yet tested at n_cycles=64, and not covered by the "7 questions answered" status above — treat it as a promising single data point, not a settled claim.**
